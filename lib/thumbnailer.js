...
Thumbnailer.prototype.execute = function(description, localPath, onComplete) {
	...
	_.extend(this, {
		...
		preParams: (description.pre || ''),
		postParams: (description.post || ''),
...
Thumbnailer.prototype.matted = function() {
	var qualityString = (this.quality ? '-quality ' + this.quality : ''),
		thumbnailCommand = config.get('convertCommand') + ' ' + this.preParams + ' "' + this.localPath + '[0]" -thumbnail ' + (this.width * this.height) + '@ -gravity center -background ' + this.background + ' -extent ' + this.width + 'X' + this.height + ' ' + qualityString + ' ' + this.postParams + ' ' + this.convertedPath;
...
Thumbnailer.prototype.bounded = function() {
	var dimensionsString = this.width + 'X' + this.height,
		...
		thumbnailCommand = config.get('convertCommand') + ' ' + this.preParams + ' "' + this.localPath + '[0]" -thumbnail ' + dimensionsString + ' ' + qualityString + this.postParams + ' ' + this.convertedPath;
...
Thumbnailer.prototype.fill = function() {
	var dimensionsString = this.width + 'X' + this.height,
		qualityString = (this.quality ? '-quality ' + this.quality : ''),
		thumbnailCommand = config.get('convertCommand') + ' ' + this.preParams + ' "' + this.localPath + '[0]" -resize ' + dimensionsString + '^ -gravity center -extent ' + dimensionsString + ' ' + qualityString + ' ' + this.postParams + ' ' + this.convertedPath;
